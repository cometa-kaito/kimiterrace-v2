...
import { fileImportEventDiffKey } from "../../src/calendar-import-key.js";
import { createDbClient, withTenantContext } from "../../src/client.js";
import {
  deleteStaleCalendarEvents,
  getCalendarEvents,
  listEnabledCalendarSources,
  mergeFileImportedEvents,
...
describeOrSkip("RLS: ADR-045 school_calendar (tenant_isolation)", () => {
  ...
  // ---- ADR-049 決定 1 追補（2026-07-12）: マージ保存（追加・更新、既存は残す）----

  it("★ mergeFileImportedEvents: キー一致は更新・新規は追加・非一致既存と iCal 行はそのまま残す", async () => {
    const srcA = await seedSource(fx.schoolA, "https://a.test/cal.ics");
    // 既存ファイル取込: 入学式（非一致 = 残る）/ 体育祭（キー一致 = 更新）/ 文化祭（trim 揺れでも一致 = 更新）。
    await seedEvent(fx.schoolA, "file:old:1", "2026-04-08", "入学式", null);
    await seedEvent(fx.schoolA, "file:old:2", "2026-09-10", "体育祭", null);
    await seedEvent(fx.schoolA, "file:old:3", "2026-10-03", "  文化祭  ", null);
    // 保護対象: iCal 由来 + orphan（マージは file: 名前空間しか読まない/消さない）。
    await seedEvent(fx.schoolA, "ical-evt", "2026-04-10", "iCal行事", srcA);
    await seedEvent(fx.schoolA, "orphan-ical", "2026-04-13", "orphan", null);
    ...
    const result = await withTenantContext(
      ...
      (tx) =>
        mergeFileImportedEvents(tx, {
          ...
          maxTotalEvents: 2000,
          events: [
            // キー一致（体育祭 9/10）→ 更新（場所が新ファイルの内容になる）。
            { summary: "体育祭", startDate: "2026-09-10", location: "グラウンド" },
            // trim 後キー一致（文化祭 10/3）→ 更新（既存側の空白揺れに防御的 trim）。
            { summary: "文化祭", startDate: "2026-10-03" },
            // 新規キー → 追加。
            { summary: "修学旅行", startDate: "2026-12-10" },
          ],
        }),
      APP,
    );
    expect(result).toEqual({ ok: true, deleted: 2, inserted: 3, keptExisting: 1 });

    await sql`RESET ROLE`;
    const rows = await sql<
      { uid: string; summary: string; location: string | null; created_by: string | null }[]
    >`
      SELECT uid, summary, location, created_by FROM school_calendar_events
      WHERE school_id = ${fx.schoolA} ORDER BY uid
    `;
    // 非一致既存（入学式）は残り、一致既存（体育祭/文化祭）は新バッチ uid に置き換わる。iCal / orphan は無傷。
    expect(rows.map((r) => r.uid)).toEqual([
      "file:merge-1:1",
      "file:merge-1:2",
      "file:merge-1:3",
    ...
    const updated = rows.find((r) => r.uid === "file:merge-1:1");
    expect(updated?.summary).toBe("体育祭");
    expect(updated?.location).toBe("グラウンド"); // 新ファイルの内容で更新されている
    expect(rows.find((r) => r.uid === "file:old:1")?.summary).toBe("入学式"); // マージの本丸 = 残る
    for (const r of rows.filter((r) => r.uid.startsWith("file:merge-1:"))) {
      expect(r.created_by).toBe(fx.userA); // 実行者を記録（ルール1）
    }
  });

  it("★ mergeFileImportedEvents: マージ後合計が上限超過なら一切書き込まず over_cap を返す", async () => {
    await seedEvent(fx.schoolA, "file:old:1", "2026-04-08", "入学式", null);
    await seedEvent(fx.schoolA, "file:old:2", "2026-09-10", "体育祭", null);
    ...
    const result = await withTenantContext(
      ...
      (tx) =>
        mergeFileImportedEvents(tx, {
          ...
          fileName: "cap.xlsx",
          actorUserId: fx.userA,
          // そのまま残る既存 2 + 今回 1 = 3 > 2 → over_cap。
          maxTotalEvents: 2,
          events: [{ summary: "修学旅行", startDate: "2026-12-10" }],
        }),
      APP,
    );
    expect(result).toEqual({ ok: false, reason: "over_cap", keptExisting: 2, incoming: 1 });

    await sql`RESET ROLE`;
    const rows = await sql<{ uid: string }[]>`
      SELECT uid FROM school_calendar_events WHERE school_id = ${fx.schoolA} ORDER BY uid
    `;
    expect(rows.map((r) => r.uid)).toEqual(["file:old:1", "file:old:2"]); // 無書き込み
  });

  it("★ mergeFileImportedEvents: 他校スコープでは既存が不可視（更新でなく追加扱い）・INSERT は WITH CHECK 拒否", async () => {
    await seedEvent(fx.schoolB, "file:b-batch:1", "2026-04-08", "他校の取込", null);

    // school A の教員 context から school B へのマージ: B の既存はキー一致でも USING で不可視 = 触れず、
    // INSERT が WITH CHECK で拒否される（replace と同じ多層防御・[[ref_drizzle_wraps_pg_error_cause_sqlstate]]
    // により bare toThrow + 下の無傷アサーションで担保）。
    await expect(
      withTenantContext(
        db,
        { schoolId: fx.schoolA, role: "teacher", userId: fx.userA },
        (tx) =>
          mergeFileImportedEvents(tx, {
            ...
            actorUserId: fx.userA,
            maxTotalEvents: 2000,
            events: [{ summary: "他校の取込", startDate: "2026-04-08" }],
          }),
        APP,
      ),
    ).rejects.toThrow();
    ...
    await sql`RESET ROLE`;
    const bRows = await sql<{ uid: string; summary: string }[]>`
      SELECT uid, summary FROM school_calendar_events WHERE school_id = ${fx.schoolB}
    `;
    expect(bRows).toEqual([{ uid: "file:b-batch:1", summary: "他校の取込" }]); // 他校は無傷
  });
});
...
describe("fileImportEventDiffKey（ADR-049 マージ/差分表示の単一キー・DB 非依存）", () => {
  it("summary は trim して比較し、null/undefined は空文字と同キーになる", () => {
    expect(fileImportEventDiffKey({ summary: "  体育祭 ", startDate: "2026-09-10" })).toBe(
      fileImportEventDiffKey({ summary: "体育祭", startDate: "2026-09-10" }),
    );
    expect(fileImportEventDiffKey({ summary: null, startDate: "2026-09-10" })).toBe(
      fileImportEventDiffKey({ startDate: "2026-09-10" }),
    );
  });

  it("summary と startDate のどちらが違ってもキーは一致しない", () => {
    const base = fileImportEventDiffKey({ summary: "体育祭", startDate: "2026-09-10" });
    expect(fileImportEventDiffKey({ summary: "文化祭", startDate: "2026-09-10" })).not.toBe(base);
    expect(fileImportEventDiffKey({ summary: "体育祭", startDate: "2026-09-11" })).not.toBe(base);
  });
});
