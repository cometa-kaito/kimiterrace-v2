...
import { getHeatAlertByArea, upsertHeatAlert } from "../../src/queries/heat-alerts.js";
...
/**
 * ADR-044: heat_alerts の RLS（公開参照マスタ特例）を実 PG で検証する（weather_warnings.test.ts の双子）。
 *
 * 検証の核（Reviewer 重点）:
 *   - **全ロール + 匿名サイネージが SELECT 可**（heat_alerts_read_all, USING (true)）。熱中症アラートは公開・非 PII。
 *     ★ ADR-016 のサイネージ匿名コンテキスト（role 未設定 / school_id のみ or 無し）が確実に読めること。
 *   - **非 system は INSERT / UPDATE / DELETE 不可**（heat_alerts_write_system_*）。書込みは system に閉じる。
 ...
 */
describeOrSkip("RLS: ADR-044 heat_alerts", () => {
  ...
  beforeEach(async () => {
    ...
    // heat_alerts は seedBaseFixture の TRUNCATE 対象外（school_id 非保持の cross-tenant 参照で
    // schools CASCADE に巻き込まれない）ため、テスト間で行が残らないよう明示クリアする。
    await sql.unsafe("TRUNCATE heat_alerts RESTART IDENTITY;");
    // 閲覧テスト用の固定行を owner 接続（RLS バイパス）で投入。
    await sql`
      INSERT INTO heat_alerts (area_code, area_name, source, forecast_date, alert_level, wbgt_max, wbgt_band, raw)
      VALUES (${AREA}, '岐阜県', 'env_moe', ${DAY}, 'warning', 33, 'danger', '{"targetDate1Flag":"1"}'::jsonb)
    `;
  });
  ...
  it("全テナントロール（school_admin/teacher/student/guardian）が SELECT できる", async () => {
    for (const role of ["school_admin", "teacher", "student", "guardian"] as const) {
      await sql.begin(async (tx) => {
        ...
        const rows = await tx`SELECT id FROM heat_alerts WHERE area_code = ${AREA}`;
  ...
  it("★ サイネージ匿名コンテキスト（role 未設定・school_id のみ）でも SELECT できる（ADR-016）", async () => {
    await sql.begin(async (tx) => {
      ...
      // 熱中症アラートは cross-tenant 共有なので school_id に関係なく heat_alerts_read_all で読める。
  ...
  it("非 system（school_admin）は INSERT できない（heat_alerts_write_system_insert）", async () => {
    await expect(
      sql.begin(async (tx) => {
        await tx.unsafe("SET LOCAL ROLE kimiterrace_app");
        ...
        await tx`
          INSERT INTO heat_alerts (area_code, source, forecast_date, alert_level)
          VALUES ('999999', 'env_moe', ${DAY}, 'warning')
        `;
      }),
  ...
  it("非 system（teacher）は UPDATE / DELETE できない（0 行 or 拒否、漏洩・改竄防止）", async () => {
    // UPDATE: heat_alerts_write_system_update の USING が false → 0 行（エラーにはならないが変更不可）。
    const updated = await sql.begin(async (tx) => {
      await tx.unsafe("SET LOCAL ROLE kimiterrace_app");
      await tx`SELECT set_config('app.current_user_role', 'teacher', true)`;
      return tx`UPDATE heat_alerts SET alert_level = 'emergency' WHERE area_code = ${AREA} RETURNING id`;
    });
    ...
    const deleted = await sql.begin(async (tx) => {
      await tx.unsafe("SET LOCAL ROLE kimiterrace_app");
      await tx`SELECT set_config('app.current_user_role', 'teacher', true)`;
      return tx`DELETE FROM heat_alerts WHERE area_code = ${AREA} RETURNING id`;
    });
    expect(deleted.length).toBe(0);

    // owner 接続で 1 行が無傷であることを確認。
    await sql`RESET ROLE`;
    const survive = await sql`SELECT alert_level FROM heat_alerts WHERE area_code = ${AREA}`;
    expect(survive.length).toBe(1);
    expect(survive[0].alert_level).toBe("warning");
  });

  // --- ★ system_admin（取得 Job 経路）の upsert ---

  it("upsertHeatAlert: system context で INSERT、競合キーで UPDATE（last-known-good 更新）", async () => {
    ...
    try {
      ...
      const rows = await client<
        {
          alert_level: string;
          wbgt_max: number | null;
          created_by: string | null;
          updated_at: Date;
          created_at: Date;
        }[]
      >`SELECT alert_level, wbgt_max, created_by, created_at, updated_at FROM heat_alerts WHERE area_code = ${NEW_AREA}`;
      expect(rows.length).toBe(1);
      expect(rows[0].alert_level).toBe("emergency");
      expect(rows[0].wbgt_max).toBe(35);
  ...
  it("getHeatAlertByArea: 匿名サイネージ context で当日以降の最新 1 行を読む / 無い地域は null", async () => {
    // biome-ignore lint/style/noNonNullAssertion: describeOrSkip で url 有り
    const client = postgres(url!, { max: 1, onnotice: () => {} });
    try {
      const db = drizzle(client);
      const found = await db.transaction(async (tx) => {
        // 匿名サイネージ context（role 無し）に降格。heat_alerts_read_all で読めることを実証。
        await tx.execute(dsql`SET LOCAL ROLE kimiterrace_app`);
        return getHeatAlertByArea(tx, AREA, DAY);
      });
      expect(found?.alertLevel).toBe("warning");
      expect(found?.wbgtMax).toBe(33);

      const missing = await db.transaction(async (tx) => {
        await tx.execute(dsql`SET LOCAL ROLE kimiterrace_app`);
        return getHeatAlertByArea(tx, "999999", DAY);
      });
