...
import { boolean, date, index, integer, pgTable, uuid, varchar } from "drizzle-orm/pg-core";
...
export const studentCallouts = pgTable(
  "student_callouts",
  {
    ...
    // 表示順（教員が呼び出し一覧を任意に並べ替える。盤面はこの昇順で描画する）。保存（全置換）時に編集 UI の
    // 行位置を 0,1,2... で採番する。既定 0（旧データ・未採番は時刻→氏名のタイブレークに委ねる）。migration 0035。
