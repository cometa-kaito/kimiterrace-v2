 ...
 *
 * 採用モデルは `gemini-embedding-001` で、既定 3072 次元を **MRL 切り詰め（`outputDimensionality = 768`）**
 * して 768 を生成する（ADR-007 追補 2026-06-01 / #396 M-2）。RAG の embedding を保存する pgvector 列
 ...
 *
 * ★ 次元の変更（例: 768 → フル 3072）は **ここ 1 箇所**を変えれば ai/db 両層に伝播する。ただし既存
 *   embedding の全件再生成 migration が必要（#396 M-2 / ADR-007 追補「再埋め込み手順」を参照）。
 *
