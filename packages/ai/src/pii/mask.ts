...
//   ① 国際 E.164（先頭 `+` 国番号 + 国内番号）。桁間の区切り（空白/ハイフン/ドット/括弧）は 0〜2 文字
//      まで許容し、総桁数を 7〜15 に bound する（E.164 上限 15 桁。住所番地・年号の誤検出を抑える）。
//      旧来の +81 もこの一般枝が内包する。
//   ② 国内（日本）ハイフン区切り（市外局番/携帯） ③ 国内 区切りなし 10〜11 桁。
// 区切り文字クラス `[-.\s()]` と `\d` は素集合なので分割位置が一意に定まり、バックトラック爆発は起きない
// （① の {0,2} / {6,14} も bound 済み）。国際番号の検出により外国籍家庭の番号が Vertex へ素通りしない。
const PHONE_RE = /\+\d(?:[-.\s()]{0,2}\d){6,14}|0\d{1,4}-\d{1,4}-\d{3,4}|0\d{9,10}/g;
