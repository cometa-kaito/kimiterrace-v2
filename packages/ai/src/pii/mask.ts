...
// 書式が決まった PII の検出パターン（F06/S4: 多言語・CJK 全角入力に拡張、ルール4）。
// すべて量化子を上限付き ({n,m}) にして線形時間で評価する（untrusted な生徒/教員入力に毎回適用する
// ため ReDoS を構造的に排除。国際電話分岐も総消費桁数が定数上限のため、入力長に対し線形）。
//
// 多言語対応の要点:
// - **氏名**は名簿 (PiiEntry) 由来のリテラル置換のため元から言語非依存（外国籍生徒名・ローマ字・
//   やさしい日本語の表記揺れも value/aliases に入れれば確実にマスクされる）。本スライスは「書式が
//   決まった PII」の検出を日本語前提から広げる。
// - **電話**: 日本の番号に加え、任意の国番号 (`+CC`) の国際表記を検出する（外国籍の生徒・保護者の番号）。
// - **全角入力 (CJK)**: 全角数字 `０-９` / 全角プラス `＋` / 全角ハイフン `－` / 全角ドット `．` /
//   全角＠ `＠` も半角と同様に検出する（日本語 IME では電話・メールが全角入力されがちで、半角前提だと
//   PII が漏れる）。検出した表層形は原文のまま辞書に載るので逆変換でラウンドトリップする。
const DIGIT = "[0-9\\uFF10-\\uFF19]"; // 半角 + 全角数字
const SEP = "[-.\\s\\uFF0D\\uFF0E\\u3000]"; // ハイフン/ドット/空白 + 全角ハイフン/全角ドット/全角空白
const PLUS = "[+\\uFF0B]"; // 半角 + 全角プラス
const AT = "[@\\uFF20]"; // 半角 + 全角アット
...
// 電話: ①国際 +CC（区切り任意）②国内 区切りあり（市外局番/携帯）③区切りなし 10〜11 桁。
// DIGIT/SEP は互いに素な文字クラスのため `(?:SEP?DIGIT{1,4}){1,4}` に曖昧さが無く、上限付き量化子で
// 総消費桁数が定数 (≤ 約 23 桁) に bound される（入力長非依存 = ReDoS 不能）。
const PHONE_RE = new RegExp(
  `${PLUS}${DIGIT}{1,3}${SEP}?${DIGIT}{1,4}(?:${SEP}?${DIGIT}{1,4}){1,4}` +
    `|${ZERO}${DIGIT}{1,4}${SEP}${DIGIT}{1,4}${SEP}${DIGIT}{3,4}` +
    `|${ZERO}${DIGIT}{9,10}`,
  "g",
);
// メール: ローカル/ドメイン/TLD を RFC 上限内に bound（局所バックトラックを定数化）。
// 全角＠ `＠` / 全角ドット `．` / 全角英字 TLD も許容（CJK IME の全角入力での漏れを塞ぐ）。
