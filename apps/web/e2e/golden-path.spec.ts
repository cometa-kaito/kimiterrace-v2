 ...
 *   1. 認証済み教員コンテキスト (storageState) で `/admin/editor/{SEED.CLASS_ID}` を開く。
 ...
 *      教員の school_id = SEED.SCHOOL_ID、SEED.CLASS_ID は同一校なので編集可。
...
test.describe("完全 golden path — 教員エディタ更新 → サイネージ反映", () => {
  ...
  test("教員がエディタで更新した連絡が公開サイネージに反映される", async ({ page }) => {
    // 1. 認証済み教員でエディタを開く (既定で JST 今日が対象日 = signage と一致)。
    await page.goto(`/admin/editor/${SEED.CLASS_ID}`);
    ...
    // 3. 公開サイネージを新規ナビゲーションで開く (force-dynamic で最新の daily_data を取得)。
