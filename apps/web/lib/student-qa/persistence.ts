 ...
 *   手書きの interface を作らない (`InferInsertModel` 経由は不要 — 直接 values オブジェクトを渡す)。
 * - ルール4 (PII): `contentText` には **PII マスキング済テキスト** のみを受け取る。マスキングは
...
export async function appendAssistantMessage(
  ...
  const [row] = await tx
    ...
    .values({
      ...
      evidence: params.evidence as unknown as object,
    })
    .returning({ id: aiChatMessages.id });
  if (!row) {
