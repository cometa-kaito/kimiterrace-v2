 ...
 * センサーの設置/設定は自校の運用管理操作であり、`MEMBER_ADMIN_ROLES` (教職員管理) と同じく
 * school_admin に限定する。teacher は閲覧のみ。system_admin の全校横断センサー操作は別面
 * (`/admin/system/sensors`、本スライス非対象) に分けるため本集合には含めない
 * ([[rls-tenant-not-role-boundary]] / advertisers・members と同じ per-surface 方針)。
