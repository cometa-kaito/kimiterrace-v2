 ...
 * ケースがあるため、行を**ドラッグ&ドロップ**で並べ替えられるようにする（{@link useRowReorder} / {@link DragHandle}・
 * 連絡 / 来校者と同部品）。並べ替えはマウス/タッチ/ペン共通のポインタ D&D で、タブレットでも掴んで動かせる。
 * フォーカス時の ↑↓ キーでも動かせる（要望 2026-06-23: 上下ボタンは廃止しドラッグ主体・キーボード経路は残す）。
 * 並べ替え後の配列順は既存の自動保存経路でそのまま保存され、サーバが各行の位置を `sort_order` に採番する
 * （migration 0035）。読み取りは `sort_order` 昇順優先（同順位は時刻→氏名）。
 */
type Row = {
...
export function CalloutsEditor({
  ...
  function moveRow(from: number, to: number) {
    setRows((prev) => {
      const dest = prev[to];
      if (!dest || isBlankCalloutRow(dest)) {
        return prev;
      }
