...
/**
 * エディタの可変フォーム行（**並べ替えた順がそのまま盤面の表示順になる**セクション）を手動で並べ替える
 * 純フック（連絡・来校者の並べ替えで使用）。`reorder(from, to)` を呼ぶと配列を組み替え、
 ...
 * - 永続化はこのフックでは行わない。呼び出し側（NoticeEditor / VisitorsEditor 等）が並べ替え後の state を
 *   **既存の保存経路**（自動保存）にそのまま流す。対象は ①配列順がそのまま保存・描画されるセクション（連絡）と
 *   ②保存時に行位置を順序列へ採番するセクション（来校者の `sort_order`・migration 0034）。サーバが
 *   period/deadline/時刻で再ソートする予定・提出物・呼び出しは対象外（盤面の表示順を変えないため）。
 *
