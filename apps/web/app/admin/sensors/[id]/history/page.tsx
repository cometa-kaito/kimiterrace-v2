...
/**
 * F13 (#391, ADR-020): 来場検知センサーの **検知履歴**ページ `/admin/sensors/[id]/history`。
 * **Server Component**。ユーザー依頼「人感センサのデータを過去のデータなど全て UI から見れるように」。
 *
 * 1 センサーの過去の検知を ①JST 日別の検知数、②最新の生検知一覧（時刻・状態）で時系列に見せる。期間は
 * `?range=`（1d/7d/30d/90d/all）で切替。一覧（俯瞰）は `/admin/sensors`、本ページは個別深掘り。
 *
  ...
  return (
    <section>
      <p style={backLinkStyle}>
        <a href="/admin/sensors" style={linkStyle}>
          ← センサー管理に戻る
        </a>
      </p>
...
function RangeLink({
  ...
  return (
    <a href={`/admin/sensors/${sensorId}/history?range=${optKey}`} style={rangeChipStyle}>
      {label}
    </a>
  );
}
