...
// `@kimiterrace/ai` を **部分 mock**: maskPII と redactSuspectedNames を差し替え可能にし、
// それ以外 (findUnmaskedPii / buildChatPrompt / classifyScope / buildScopeRefusal …) は
// importActual spread で実物を保持する。
//
// ISSUE-1(b) 追記: UIUX-03/C ブランチで chat-service に `redactSuspectedNames` (best-effort 伏字)
// が追加された。maskPII を素通しにしても redactSuspectedNames が氏名を伏字化するため、
// fail-closed 経路のケースでは redactSuspectedNames も素通しに差し替えて findUnmaskedPii まで
// PII を届ける必要がある。対称性ケース (実 maskPII が通る) では両方とも実物のまま。
vi.mock("@kimiterrace/ai", async (importActual) => {
  ...
  return {
    ...
    redactSuspectedNames: vi.fn(actual.redactSuspectedNames),
  };
});

import { type PiiEntry, maskPII, redactSuspectedNames } from "@kimiterrace/ai";
...
beforeEach(async () => {
  ...
  vi.clearAllMocks();
  // maskPII / redactSuspectedNames の既定挙動を「実物」に戻す。`vi.mock` 工場で
  // `vi.fn(actual.xxx)` を初期実装にしているが、500 ケースは mockImplementation で
  // 「原文素通し」に上書きするため、対称性ケースが実物を確実に通すよう毎ケース冒頭で再設定する。
  ...
  vi.mocked(redactSuspectedNames).mockImplementation(actual.redactSuspectedNames);
...
describe("executeChat: PII マスク漏れ fail-closed (500 pii_leak) の決定的 pin", () => {
  it("maskPII が原文を素通しすると実 findUnmaskedPii が氏名を検出し 500 pii_leak、モデルは呼ばれない", async () => {
    ...
    //
    // ISSUE-1(b): chat-service では maskPII 後に redactSuspectedNames (best-effort 伏字) が呼ばれる。
    // maskPII 素通し単体では redactSuspectedNames が氏名を伏字化してしまい findUnmaskedPii に届かない
    // ため、fail-closed 経路の決定的 pin には redactSuspectedNames も素通しに差し替える必要がある。
    vi.mocked(maskPII).mockImplementation((text: string) => ({ masked: text, dictionary: {} }));
    vi.mocked(redactSuspectedNames).mockImplementation((text: string) => text);
  ...
  it("漏れが context 本文のみでも (質問は無害) fail-closed 500 になりモデルを呼ばない", async () => {
    ...
    // ISSUE-1(b) 同様、fail-closed 経路を決定的に踏むため redactSuspectedNames も素通しに差し替える。
    vi.mocked(maskPII).mockImplementation((text: string) => ({ masked: text, dictionary: {} }));
    vi.mocked(redactSuspectedNames).mockImplementation((text: string) => text);
...
/** stream 結果を消費し、表示された全テキストを連結して返す (done まで待ち永続化も確定させる)。 */
async function collectStream(result: Awaited<ReturnType<typeof executeChat>>): Promise<string> {
  if (result.kind !== "stream") return "";
  let out = "";
...
describe("executeChat: モデル *出力* の PII 再チェック (多層防御・出力 fail-closed)", () => {
  it("出力に電話/メールが混入したら表示を打ち切り、保存も安全な定型文に倒す (生 PII を生徒/DB に残さない)", async () => {
    // 入力 (質問・context) は無害。モデルが辞書化漏れ・ハルシネーションで生の電話番号を出力する状況を
    // フェイク client で再現する (出力は maskPII を通らない = 入力ゲートでは捕捉できない経路)。
    const { client } = makeModelClient({
      chunks: ["お電話は", "090-1234-5678", "です。"],
      fullText: "お電話は090-1234-5678です。",
    });
    const result = await executeChat(
      baseParams({
        modelClient: client,
        piiEntries: [], // 電話はパターン検出 (名簿不要)。
        rawQuestion: "連絡先は掲示で確認できますか？",
        contextProvider: vi.fn(async () => [
          { id: "c1", title: "お知らせ", body: "連絡先は職員室の掲示にあります。" },
        ]),
      }),
    );
    ...
    expect(result.kind).toBe("stream");
    if (result.kind !== "stream") return;
    const shown = await collectStream(result);

    // 1) 表示: 電話番号が揃ったチャンクで打ち切られ、生の電話番号は生徒に届かない。
    expect(shown).not.toContain("090-1234-5678");
    // 2) 保存: 生 PII でなく安全な定型文を、出力伏字センチネル model_version で永続化する。
    expect(appendAssistantMessage).toHaveBeenCalledTimes(1);
    const saved = vi.mocked(appendAssistantMessage).mock.calls[0]?.[1] as {
      maskedText: string;
      modelVersion: string;
      confidenceScore: number;
    };
    expect(saved.maskedText).not.toContain("090-1234-5678");
    expect(saved.maskedText).toBe("内部エラーにより応答を表示できませんでした。");
    expect(saved.modelVersion).toBe("pii-redacted-output");
    expect(saved.confidenceScore).toBe(0);
  });

  it("対称性 (非空虚性): 出力が無害なら全文を表示し原文のまま保存する", async () => {
    const { client } = makeModelClient({
      chunks: ["体育祭は", "6/10 開催です。"],
      fullText: "体育祭は6/10 開催です。",
    });
    const result = await executeChat(
      baseParams({
        modelClient: client,
    ...
    // 無害出力は打ち切らず全文表示・原文保存 (出力ゲートが正常出力を壊さないこと)。
    expect(shown).toBe("体育祭は6/10 開催です。");
    const saved = vi.mocked(appendAssistantMessage).mock.calls[0]?.[1] as {
      maskedText: string;
      modelVersion: string;
    };
    expect(saved.maskedText).toBe("体育祭は6/10 開催です。");
    expect(saved.modelVersion).toBe("gemini-test-001");
  });
});
