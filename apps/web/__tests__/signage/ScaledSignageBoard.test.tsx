...
describe("PR-B 規定超過の自動スクロール（汎用機構 §11a の構造ガード）", () => {
  ...
  /**
   * pattern2（予定/来校者/呼び出し）は **JS `AutoScroll`（自然高さ・Element.animate）** でラップされる（2026-06-23
   * ユーザー決定。固定高 CSS 版だと 2 行アイテム＝氏名＋用件メタが 5 件以下でもクリップしうるため）。よって汎用
   * CSS 版（§11a）の inline マーカー（`--visible-rows` / `--total-rows`）も固定高クラス（`.autoScrollItem`）も**持たない**。
   * jsdom はレイアウト計測が 0 で JS AutoScroll は静的フォールバック＝**全件が DOM に残る**（自然高さでクリップ無し）。
   * ここでは「CSS 版マーカーを持たない（＝JS 版でラップされた）」ことだけを構造で担保する（視覚スクロールは実機確認）。
   */
  function expectNoCssAutoScrollMarkers(region: HTMLElement): void {
    expect(region.querySelector('[style*="--visible-rows"]')).toBeNull();
    expect(region.querySelector('[style*="--total-rows"]')).toBeNull();
  }
  ...
  it("pattern2 予定列: JS AutoScroll でラップ＝CSS 版マーカー無し、6 コマ全件が DOM に残る", () => {
    render(
      <SignageBoardView
        {...boardProps(
    ...
    const region = screen.getByRole("region", { name: "予定" });
    // CSS 版（§11a）の固定高マーカーは持たない＝JS AutoScroll（自然高さ）でラップされている。
    expectNoCssAutoScrollMarkers(region);
    // 超過コマも全件 DOM に残る（自然高さでクリップせずスクロールで見せる）。
    expect(region).toHaveTextContent("コマ1");
    expect(region).toHaveTextContent("コマ6");
  });

  it("pattern2 来校者: JS AutoScroll でラップ＝CSS 版マーカー無し、6 件 + 用件メタ全件が DOM に残る", () => {
    render(
      <SignageBoardView
    ...
    expectNoCssAutoScrollMarkers(region);
    // 氏名（主行）も用件（メタ 2 行目）も全件 DOM に残る＝自然高さで 2 行アイテムをクリップしない前提。
    expect(region).toHaveTextContent("来校者6");
    expect(region).toHaveTextContent("用件6");
  });

  it("pattern2 呼び出し: JS AutoScroll でラップ＝CSS 版マーカー無し、6 件 + 用件メタ全件が DOM に残る", () => {
    render(
      <SignageBoardView
