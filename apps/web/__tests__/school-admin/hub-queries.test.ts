...
describe("reduceTodayActiveScopes（窓行 → 今日掲示中の scope 集約）", () => {
  ...
  it("固定行 (pinned・§5.4): 14 日をはるかに超えた過去入力でも今日も active (盤面と同じ活性判定の単一ソース)", () => {
    // getDailyWindowRows は pinned を含む行を窓の外でも返す (JSONB 包含 OR・packages/db 側で検証)。
    // ここではその行が届いた前提で、活性集約が pinned を「ずっと掲示中」と判定することを固める。
    const SIXTY_DAYS_AGO = "2026-05-05";
    expect(
      reduceTodayActiveScopes([
        wrow("class", {
          classId: "c1",
        ...
        // pinned でない同日の古い連絡だけの行は非活性のまま。
        wrow("class", {
          classId: "c2",
          date: SIXTY_DAYS_AGO,
          notices: [{ text: "昔の連絡", displayDays: 14 }],
        }),
      ]),
    ).toEqual({ school: false, departmentIds: [], gradeIds: [], classIds: ["c1"] });
  });
