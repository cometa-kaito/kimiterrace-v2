...
describe("ScheduleEditor 事前生成した空行は保存をブロックしない / 落とす", () => {
  ...
  it("1 行だけ埋めると空行を落として 1 件だけ保存する", async () => {
    ...
    // 同一 tick で時限も選ぶ → debounce が 1 回に集約され period 付き 1 件で保存（科目のみ保存は別テストで固定）。
  ...
  it("事前生成行の時限は未選択（空欄）で始まり、科目だけでも保存する（時限なし＝科目のみ・時限を選ぶと時限付きで保存）", async () => {
    render(
      <ScheduleEditor
        classId={CLASS_ID}
        date={DATE}
    ...
    // 科目だけ入れる → 時限なし（科目のみ）の予定として保存する（period を持たない・要望 2026-06-23: 科目のみで
    // 盤面に表示できるように）。盤面は時限ラベルなしで科目だけを描く。
    fireEvent.change(screen.getByLabelText("1 行目の科目名"), { target: { value: "数学" } });
    await act(async () => {
      await vi.advanceTimersByTimeAsync(800);
    });
    expect(h.setScheduleAction).toHaveBeenCalledTimes(1);
    const first = savedItems(h.setScheduleAction) as Array<Record<string, unknown>>;
    expect(first).toHaveLength(1);
    expect(first[0]?.subject).toBe("数学");
    // 時限なし＝period を持たない（番兵 0 を wire に載せない）。
    expect(first[0] && "period" in first[0]).toBe(false);
    // 時限を選ぶと時限付きで保存し直す。
    fireEvent.change(screen.getByLabelText("1 行目の時限"), { target: { value: "2" } });
    await act(async () => {
      await vi.advanceTimersByTimeAsync(800);
    });
    expect(h.setScheduleAction).toHaveBeenCalledTimes(2);
    const secondCall = h.setScheduleAction.mock.calls[1] ?? [];
    const second = (secondCall.find((a) => Array.isArray(a)) as Array<{ period: number }>) ?? [];
    expect(second[0]?.period).toBe(2);
  });
