...
describe("ScheduleEditor 事前生成した空行は保存をブロックしない / 落とす", () => {
  ...
  it("1 行だけ埋めると空行を落として 1 件だけ保存する", async () => {
    ...
    // 事前生成行の時限は 1 限から自動採番されるので、科目だけ入れると即保存する（要望 2026-06-23）。
  ...
  it("事前生成行の時限は 1 限から自動採番され、科目だけで保存できる（要望 2026-06-23）", async () => {
    render(
      <ScheduleEditor
        classId={CLASS_ID}
        date={DATE}
    ...
    // 事前生成行の時限は 1・2・3… と自動採番（行の位置＝時限として盤面に出る）。
    expect((screen.getByLabelText("1 行目の時限") as HTMLSelectElement).value).toBe("1");
    expect((screen.getByLabelText("2 行目の時限") as HTMLSelectElement).value).toBe("2");
    expect((screen.getByLabelText("3 行目の時限") as HTMLSelectElement).value).toBe("3");
    // 2 行目の科目だけ入れると、時限=2 として保存される。
    fireEvent.change(screen.getByLabelText("2 行目の科目名"), { target: { value: "英語" } });
    await act(async () => {
      await vi.advanceTimersByTimeAsync(800);
    });
    expect(h.setScheduleAction).toHaveBeenCalledTimes(1);
    const items = savedItems(h.setScheduleAction) as Array<{ period: number; subject: string }>;
    expect(items).toHaveLength(1);
    expect(items[0]?.period).toBe(2);
    expect(items[0]?.subject).toBe("英語");
  });
