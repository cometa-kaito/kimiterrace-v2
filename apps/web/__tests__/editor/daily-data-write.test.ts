...
import type { EditorTarget, ScopedEditorActor } from "../../lib/editor/schedule-core";
...
/** tenant ロール (school_admin / teacher): userRef=自身の users.id、identityUid=null。 */
const actor: ScopedEditorActor = {
  actorUserId: USER_ID,
  userRef: USER_ID,
  identityUid: null,
  schoolId: SCHOOL_ID,
...
/** system_admin: users 行が無いため userRef=null (FK 回避)、identityUid=IdP uid。schoolId は対象校。 */
const sysActor: ScopedEditorActor = {
  actorUserId: SYS_UID,
  userRef: null,
  identityUid: SYS_UID,
  schoolId: SCHOOL_ID,
...
describe("upsertDailySectionForTarget: INSERT カラムマッピング (scope ごと)", () => {
  for (const c of cases) {
    it(`${c.name} scope は正しい scope + *_id 列を INSERT する`, async () => {
      ...
      // 監査ログを daily_data 対象で同 tx 追記 (ルール1)。tenant ロールは actor_user_id=自身、
      // actor_identity_uid=null、created_by/updated_by=自身の users.id。
...
/**
 * system_admin (users 行なし) の FK 安全性 (ルール1 / system-admin-not-in-users-table)。
 * daily_data の created_by/updated_by と audit_log の created_by/updated_by に **uid を入れると
 * FK 違反 (23503)** するため、`userRef=null` が INSERT/UPDATE/audit に渡ることを fake tx の引数で固定する。
 * 追跡は audit_log.actor_user_id (FK 無し) + actor_identity_uid (IdP uid) で担保する。
 */
describe("upsertDailySectionForTarget: system_admin の FK 安全 (createdBy/updatedBy=null)", () => {
  it("INSERT: daily_data と audit_log の created_by/updated_by が null、audit に identityUid を残す", async () => {
    const cap = emptyCapture();
    const id = await upsertDailySectionForTarget(
      asTx(fakeTx(null, cap)),
      sysActor,
  ...
  it("UPDATE: updated_by が null、audit も created_by/updated_by=null + identityUid を残す", async () => {
    const cap = emptyCapture();
    const id = await upsertDailySectionForTarget(
      asTx(fakeTx({ id: "daily-existing", notices: [] }, cap)),
      sysActor,
    ...
    expect(id).toBe("daily-existing");
    expect(cap.updateSet).toMatchObject({ updatedBy: null });
    expect(cap.auditValues).toMatchObject({
      operation: "update",
      actorUserId: SYS_UID,
      actorIdentityUid: SYS_UID,
