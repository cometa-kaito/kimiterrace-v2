...
describe("CalloutsEditor Tab 縦移動（呼び出し・氏名=列0 / 呼出先=列1 / 用件=列2）", () => {
  const items = () =>
    [
  ...
  it("用件で Tab → 次の行の用件へ（列 2）", () => {
    render(<CalloutsEditor classId={CLASS_ID} date={DATE} initialItems={items()} />);
    input("1 行目の用件").focus();
    fireEvent.keyDown(input("1 行目の用件"), { key: "Tab" });
    expect(document.activeElement).toBe(input("2 行目の用件"));
  });
...
describe("VisitorsEditor Tab 縦移動（来校者・氏名=列0 / 所属1 / 用件2 / 対応者3 / 備考4）", () => {
  const items = () =>
    [
      {
        scheduledTime: "10:00",
  ...
  it("対応者で Tab → 次の行の対応者へ（列 3・列がずれていない）", () => {
    render(<VisitorsEditor classId={CLASS_ID} date={DATE} initialItems={items()} />);
    input("1 行目の対応者").focus();
    fireEvent.keyDown(input("1 行目の対応者"), { key: "Tab" });
    expect(document.activeElement).toBe(input("2 行目の対応者"));
  });
  ...
  it("最終行で Tab → 新規行を追加して同じ列へ", () => {
    render(<VisitorsEditor classId={CLASS_ID} date={DATE} initialItems={items()} />);
    input("2 行目の備考").focus();
    fireEvent.keyDown(input("2 行目の備考"), { key: "Tab" });
    expect(document.activeElement).toBe(input("3 行目の備考"));
  });
});
