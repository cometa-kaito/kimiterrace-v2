...
describe("navItemsForRole", () => {
  it("teacher はエディタのみ (2026-06-07 ユーザー判断: 先生の工数を増やさない原則で導線を絞る。音声/チャット入力/コンテンツ/掲示物 Q&A は機能残置・URL直打ち可。監視系は校務DX原則で運営専用に撤去、MFA は意図的に nav から撤去)", () => {
  ...
  it("掲示物 Q&A (/admin/chat) は school_admin のみ nav に出す。system_admin は nav 非表示 (F06 #370、死リンク防止)。teacher は 2026-06-07 判断で nav から撤去 (機能・認可は残置、URL直打ち可)", () => {
    ...
    // teacher は機能として使えるが nav 導線は「エディタのみ」方針で撤去 (2026-06-07)。
  ...
  it("音声/チャット入力 (/admin/teacher-input) は school_admin のみ nav に出す。system_admin は nav 非表示 (TEACHER_INPUT_STAFF_ROLES と整合・死リンク防止)。teacher は 2026-06-07 判断で nav から撤去 (機能・認可は残置、URL直打ち可)", () => {
  ...
  it("コンテンツ (/admin/contents) は school_admin のみ nav に出す。system_admin は nav 非表示 (#166 と整合・死リンク防止)。teacher は 2026-06-07 判断で nav から撤去 (機能・認可は残置、URL直打ち可)", () => {
