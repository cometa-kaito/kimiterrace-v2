...
locals {
  ...
  # ★ 注意: 本 image は「未コミット NDJSON を含む特定ビルド」であり、コミット a165780 のクリーン
  #   チェックアウトからの再ビルドでは NDJSON 不在で Job が runtime fail する（在室時刻を公開 repo に
  #   残さない方針とのトレードオフ）。再取り込み時は LP から再ダンプ → 再ビルドして本タグを bump する。
  ...
  web_image_tag = "f48b86f" # 2026-06-20: 時事ニュース・連絡(pattern4)を縦オートスクロール化し長文/要約の文字切れを解消（#1094）。中間 #1093（エディタのモニタ解決を signage_url トークンでも・tv-devices クエリ層）も同梱。表示層(apps/web)のみ・schema/migration なし・secret 変更なし。疎通 /api/health 200・/login private,no-cache（s-maxage 退行なし）
}
...
# 実行: `gcloud run jobs execute kimiterrace-seed-ginan-sch --region asia-northeast1 --project signage-v2-staging`。
...
# 実行: `gcloud run jobs execute kimiterrace-bf-presence --region asia-northeast1 --project signage-v2-staging`。
...
# staging は AI 検証環境（cloud_run web 同様 ai_enabled=true）。停止は ai_enabled=false で即 OFF。
#
...
module "cloud_run_job" {
  ...
  ai_enabled             = true  # staging は AI 検証環境（web cloud_run と同方針）。停止は false に戻して apply で即 OFF
...
# pattern2/3 サイネージ工学ニュース取得 Job（政府系 / JST の公開 RSS、ADR-043）。30 分間隔。
# PR2 は **enabled=false（scaffold）**。有効化時に jobs image を news-job 同梱で build/push 後、enabled=true へ
# flip して apply する（image が dist/news/news-job.js を含む前に Job を作らない）。image / vpc_connector /
# database_url_secret_id は weather / railway と同じローカル変数を流用（新規 secret なし・RSS はキーレス）。
