...
locals {
  ...
  web_image_tag = "01c5f6a" # 2026-06-13 deploy: サイネージ pattern1 天気をアイコンのみ化(#847) 反映（schema 無変更=migrate 不要）
}
...
# 供給する。staging 有効化（2026-06-13）: prod と同形で先に検証する。school_admin が公開した
# content_versions を毎時走査し、PII マスク後テキストから embedding を生成して pgvector に投入する。
#
...
module "cloud_run_job" {
  ...
  region                 = var.region
  env                    = local.env
  enabled                = true # 2026-06-13 有効化: 生徒/保護者 Q&A の知識源供給（ADR-038）。jobs image 同梱・network/secret 準備済
