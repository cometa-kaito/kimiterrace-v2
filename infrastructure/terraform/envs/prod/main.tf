...
locals {
  ...
  web_image_tag = "f48b86f" # 2026-06-20: pattern4 のエディタ↔実機パターン不一致を是正（#1093 エディタのモニタ↔クラス解決を signage_url の magic_link トークン経由にも対応・class_id 列 NULL の手貼り端末でも実機どおりのパターンを解決）＋ 時事ニュース/連絡を縦オートスクロール（#1094）。土台 2b5d7c1 込み。表示層(apps/web)/packages/db query のみ・schema/migration なし・secret 変更なし。疎通 /api/health 200・/login private,no-cache（s-maxage 退行なし）。staging も同 sha 反映済
}
