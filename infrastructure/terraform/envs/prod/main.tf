...
locals {
  ...
  web_image_tag = "94ebf01" # 2026-07-22: 教員エディタ 日付バーの余白を不透明化 #1322（#1319 の 0.5rem 余白が透明で本文が透け「チラチラ」した指摘対応・バーをヘッダ直下フラッシュ＋余白を padding-top にして白背景で覆う・CSS のみ）。**別レーン #1321(PlanToolsMenu) は意図的に非含（main tip は 1acd51cd だが prod はこの #1322 まで）**。schema・secret 無変更=migrate 不要。疎通 /api/health=200・/login cache-control=private,no-cache（s-maxage 退行なし）
}
