...
locals {
  ...
  web_image_tag = "566534a" # prod deploy: #1032 TVデバイス削除（ソフトデリート）UI（編集画面に「危険操作→削除」）。schema/secret 無変更=migrate 不要（既存 deleted_at 列を利用）。疎通 /api/health 200 + /login private,no-cache
}
