...
locals {
  ...
  web_image_tag = "8908905" # 2026-07-05: 教員エディタ配置最適化（#1230・盤面プレビュー左 sticky＋編集右カラムの 2 カラム化・来校者/呼び出しを編集カラムに内包し container query で畳み・配置のみで機能不変）。schema 非変更=migration 不要・secret 無変更。疎通 health200/login private,no-cache。※実 prod は 4ce9594 稼働だったが committed 値が 1bed55f のまま残っていた（別スレの bump コミット漏れ）ため本コミットで実態=8908905 に同期
}
