...
- IAM database authentication（passwordless DB 認証＝`google_sql_user` type `CLOUD_IAM_SERVICE_ACCOUNT`）。
  現状は Secret Manager の人間投入パスワードを使う built-in 認証ゆえパスワードが Terraform state に入る
  （`modules/cloud_sql` 参照）。app 接続方式確定後にパスワードレス化する。**prod cutover の gate**。
...
以下は [`.github/workflows/terraform.yml`](../../.github/workflows/terraform.yml) で
`infrastructure/terraform/**` を触る PR の CI ゲートとして自動実行される（#438）。
ローカルでも同じコマンドで再現できる:
...
> `terraform plan` の CI 実行（GCS backend + WIF creds 必須）は別 follow-up。
> 雛形段階（`enabled = false`）は plan が "No changes" になる設計のため、CI は fmt/validate に絞る。
