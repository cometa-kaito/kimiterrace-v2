...
- **ユーザー provisioning 規約（localId = `users.id`）**: firebase-admin の `verifySessionCookie().uid` は常に **Auth の localId（= ID トークンの `sub`）** を返し、`uid` という名の custom claim は予約衝突で**無視される**（PR [#224](https://github.com/cometa-kaito/kimiterrace-v2/pull/224) の検証で実証）。したがって DB の user id を claim 経由でトークンに載せる経路は存在しない。Identity Platform ユーザーを作成する際は、特権 SA が **localId 自体を DB の `users.id`（UUID）に一致させて provisioning する**（`createUser({ uid: <users.id> })` 相当で localId を明示指定）。トークンには `role` / `school_id` のみを custom claim として付与する。`apps/web/lib/auth/session.ts` の `normalizeClaims` が `decoded.uid` を UUID 必須としているのはこの規約を前提とした deny-by-default であり、claim へ DB id を二重格納しない（[CLAUDE.md ルール3](../../CLAUDE.md) の型単一ソース思想と整合）。
