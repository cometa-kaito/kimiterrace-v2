...
## 追記: 推定原因の表示分類（運営整理 Phase6 / BUG-2 切り分け加速、2026-06-14）

ダウンタイム履歴 2 ビュー（`/ops/tv-downtime` 全校横断・`/ops/tv-devices/[deviceId]/history` デバイス単位）に、各アウテージの**推定原因を表示する診断レイヤー**を追加した（共通ロジック = `apps/web/lib/tv/downtime-cause.ts` の `estimateDowntimeCause`）。本追記はその意思決定と既知の限界を記録する。

- **表示専用・死活判定は不変**: 本分類は記録済の行を**読み取って表示時に推定するだけ**で、死活判定（`tv-liveness.ts`、上記「誤報」改訂 #851 の OFF 時間帯評価凍結）・ダウンタイム書き込み経路・`cause_hint` の永続化には一切触れない。
- **区別できないものは断定しない**: 本 ADR §悪い影響のとおり電源OFF / ネット断 / アプリ停止は心拍だけでは区別不能。よって分類器は区別可能なものだけ確定（`reboot` = 永続 `cause_hint`、`scheduled_off` = 消灯窓内）し、区別不能なケースは `indeterminate`（応答途絶・未確定）に倒して**候補 3 つ（電源OFF / ネット断 / アプリ停止）を併記**する。安全側・誤った断定をしない（公立校データの運用方針）。
- **2 層トラスト**: per-row・復帰時に凍結された事実（`cause_hint='reboot'`・`went_down_at`）を**権威**とし、デバイスの**現在の** `schedule_json` は soft context として扱う。reboot は per-row 永続値からのみ取り、**現在の `last_boot_at` は分類器に渡さない**（古い行を「昨日の再起動」で誤分類しないため）。
- **既知の限界（スケジュール現在値）**: 過去行の `scheduled_off` 判定はデバイスの**現在**の `schedule_json` を使う近似で、学校が後にスケジュールを変更していればズレうる。緩和として (a) reboot（確定事実）が常に schedule より優先、(b) 表示の根拠文に「現在設定基準・過去設定とは異なる場合あり」と明示。per-row のスケジュール・スナップショット永続化は schema/書込経路変更になるため採らない（PoC 診断には過剰）。
- **公開契約化**: `isSignageOffHours`（本 ADR の純関数）を `apps/web` が `@kimiterrace/db` 経由で再利用するため、JST OFF 時間帯判定の de-facto 公開 API になった（単一ソース維持が目的。今後の `tv-liveness.ts` リファクタはこの export を壊さないこと）。
