...
| 038 | [生徒/保護者向け Q&A(RAG) の知識源と embedding バッチの本番有効化](038-student-qa-rag-knowledge-source.md)（知識源=school_admin が `/admin/contents` で公開した published content_versions。盤面再利用/教員 teacher-input 残置を不採用。embedding バッチ Job を staging/prod で有効化＝初回起動が冪等バックフィル、PII マスク境界不変、空知識は general_supplement フォールバック。item4） | Accepted |
