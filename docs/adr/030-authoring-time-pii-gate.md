...
- **#426 は本 ADR + 後続コードスライスで解決に向かう**（本 ADR は方針確定 = Proposed→Accepted を Reviewer 検証で確定）。
  - 次スライス（コード）: `packages/ai` に決定論ヒューリスティック検出（純関数 + テスト）、apps/web publish Server Action に
    soft-gate 配線 + override 監査、authoring UI 注意喚起。`packages/db` の content 書込み関数（`contents-publish.ts` の
    `publishContent`/`updateContent`/`rollbackContent`）自体は非変更（上流の apps/web Server Action で gate）。
