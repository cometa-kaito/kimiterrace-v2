...
- **#426 は本 ADR + コードスライスで実装着地（2026-06-03）**:
  ...
  - ✅ コンテンツポリシー明文化は [`docs/compliance/embedding-pii-masking.md`](../compliance/embedding-pii-masking.md)
    （#433）+ 本 ADR。
- **F03 AI 構造化の authoring 経路にも soft-gate を適用済（#289 ③、実 Vertex 有効化の受入ゲート）**:
  `apps/web/lib/ai/extract-teacher-input.ts` が Vertex 送信前に transcript へ `findSuspectedPersonalNames` を
  走らせ、検出 & 未 override なら送信せず `pii_warning`（route → **HTTP 409** + `suspectedSurfaces`）を返す。
  教員が承知の上で `acknowledgePii=true` で再送した場合のみ、**送信前に件数のみ** `audit_log` に記録
  （operation=update・diff=`{aiExtractPiiOverride, suspectedNameCount, kind}`、生氏名は複製しない＝ルール4。
  監査が落ちたら送信しない fail-safe）してから実行する。これで §1 の「F03 にも同 gate を適用する範囲」を確定。
  実 Vertex 呼び出し全体は別途 `AI_ENABLED` kill-switch（#289 ①、route/action 境界）で gate される。
