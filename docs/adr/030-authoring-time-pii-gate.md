...
1. **authoring 経路の検出ガード（soft-gate）**: 掲示物 publish の Server Action（`packages/db` の
   `insertContentVersion` を呼ぶ上流、apps/web）で、本文に対し**決定論的な高確信パターン**を走査する。
...
- **#426 は本 ADR + 後続コードスライスで解決に向かう**（本 ADR は方針確定 = Proposed→Accepted を Reviewer 検証で確定）。
  - 次スライス（コード）: `packages/ai` に決定論ヒューリスティック検出（純関数 + テスト）、apps/web publish Server Action に
    soft-gate 配線 + override 監査、authoring UI 注意喚起。`packages/db` の `insertContentVersion` 自体は非変更（上流で gate）。
  - `docs/compliance/` のコンテンツポリシー明文化。
