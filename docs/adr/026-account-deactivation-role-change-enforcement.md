...
- **推奨根治（実装は packages/db レーンへ）**: 「各校に有効な `school_admin` ≥1」を **DB レベルの不変条件**（トリガ or 制約関数の migration）で保証するのが本命。partial unique index では表現できない。代替は count と UPDATE を同一 tx + `SELECT … FOR UPDATE` で直列化するが、IdP-first 順序との両立設計が要る。**D2 降格に着手する前に方式を確定する**のが効率的。
