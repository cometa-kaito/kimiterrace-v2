...
- **短いパスワードの総当たり**: 4 文字許容のため総当たり耐性は弱い。緩和: ①ログイン route で**失敗回数のみ**を IP 単位でレート制限（成功＝学校 NAT 共有の一斉ログインは非計上）、②理由を畳んだ 401（学校/パスワードの列挙を防ぐ）、③volume の最終防壁は WAF/Cloud Armor、④UI で長めのパスワードを推奨。将来、より強い MFA/長さ強制が必要なら別 ADR。
