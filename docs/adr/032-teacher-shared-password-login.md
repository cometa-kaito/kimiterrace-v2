...
## 追補（2026-06-23）: 学校選択の廃止 = 入力パスワードで学校を自動判定

- Status: Accepted（本 ADR の決定を**変更せず実装を簡素化**する追補）
- Deciders: ユーザー（運営）/ Claude

### 背景

当初は「共通ログイン有効校が複数あるとき**学校選択 UI**を出す」設計だった（1 校なら選択省略）。運営から「教員は**学校選択も面倒**。パスワードだけでログインしたい」という要望（2026-06-23）。

### 決定

ログイン画面から**学校選択 UI を廃止**し、教員は**パスワードのみ**を入力する。サーバー（`/api/auth/teacher-login`）は、共通ログイン有効校それぞれの共通教員アカウントへ**入力パスワードを並列に試行**（`signInWithPassword`）し、**ちょうど 1 校だけ認証成功**したらその学校でログインする。

- **0 校一致** → `invalid_credentials`（401、失敗 1 件計上）。
- **2 校以上一致**（= **学校間でパスワードが重複**）→ どの校か特定不能 = **テナント越境のリスク**。安全側で**ログインを拒否**（`ambiguous` → 401）し、運用是正のため `logger.warn`（学校 id のみ・秘密/PII なし）を残す。
- 試行先は IdP 呼び出し増幅の安全弁として **`MAX_AUTODETECT_SCHOOLS`（=25）校**で頭打ち（現実規模は 1〜数校）。
- クライアントへ渡すのは「有効校が 1 校以上あるか」の真偽値のみ（学校 id/名は公開ログイン画面に出さない）。

### 新たなセキュリティ上のトレードオフと緩和

- **学校間パスワード重複 = テナント越境リスク**（新規）。共通パスワードが 2 校で一致すると「どちらの校か」を入力だけでは決められない。緩和: ①**運用前提として共通パスワードを全校でユニーク**にする、②一致が 2 件以上なら**ログインさせず拒否**（先勝ちで誤った校に通さない）+ warn でオペレーションに是正を促す。誤って重複させた場合、当該 2 校は**両方ともログイン不能**になる（フェイルクローズ）。
- **試行回数の増幅**: 1 リクエストで最大 `MAX_AUTODETECT_SCHOOLS` 回の IdP 往復が起きるが、**失敗計上は 1 リクエスト = 1 件**（既存の IP 失敗レート制限はそのまま頭打ち）。スケールして有効校が `MAX_AUTODETECT_SCHOOLS` を超える運用が現実化したら、per-school ログイン URL（URL で学校を固定）等へ移行する別 ADR を立てる。

### 代替案（不採用）

- **学校ごとの専用ログイン URL**（URL/サブドメインで学校を固定し、パスワードのみ）: 越境リスクが無く最も安全だが、学校ごとのブックマーク/リンク配布が要る。現規模（少数校）では自動判定の UX 単純さを優先し**将来オプション**として保留。
