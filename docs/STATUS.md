...
- **main HEAD**: `f042fe8`（#895 namespace 改称 PR3/3・`/admin` 物理撤去）。URL は `/ops`(運営)・`/app`(学校) の 2 namespace、旧 `/admin/*` は 308 で温存（[[ref_namespace_rename_breaks_e2e_url_asserts]] / 下記「直近の完了」2026-06-14）。
- **staging live**: image `web:f042fe8`（2026-06-14・namespace 改称反映）。`/api/health` 200 / `/login` は `private,no-cache`。旧 `/admin/*`→`/ops`・`/app` の **308 実機疎通済**。`migrate_image_tag` 据置（schema 変更なし）。デプロイ手順は [web-deploy.md](runbooks/web-deploy.md)。**prod 未反映**。
...
- 2026-06-14: **namespace 改称 §4.1 完了（/admin → /ops・/app）+ staging deploy・実機疎通済**。経路設計実装設計書 §4.1/§42.5（ユーザー承認 2026-06-14）に基づき v2 管理 URL を 2 namespace に物理改称: `/admin/system/*`→`/ops/*`（運営・配信、[#891](https://github.com/cometa-kaito/kimiterrace-v2/pull/891)）/ 残り `/admin/*`→`/app/*`（学校、[#894](https://github.com/cometa-kaito/kimiterrace-v2/pull/894) 中核 + [#895](https://github.com/cometa-kaito/kimiterrace-v2/pull/895) 残り+`/admin` 物理撤去）。`/admin` は消滅。旧 URL は `next.config.ts` の **308 redirect 2本**（`/admin/system/:path*`→`/ops`【先】・`/admin/:path*`→`/app`【後・catch-all】、**first-match-wins の順序が肝**）で温存。各 PR fresh Reviewer APPROVE + CI 13 green + 自律 merge。staging を `web:f042fe8` へ deploy し **実機 308 疎通済**（`/admin/system/schools`→`/ops/schools`・`/admin/editor`→`/app/editor`・`/admin`→`/app`、新 routes は `/login` へ gate=404 無し）。**prod 未反映**（`scripts/deploy/deploy-web.sh prod --apply` で同手順・schema 無変更=migrate 不要）。運営4ビュー(dashboard/sensors/reports/tv-devices)の `/ops` 化は重複統合(§43)後続、portal `rebounder.jp/admin`(商流)は §4.1 現状維持で不変。教訓は [[ref_namespace_rename_breaks_e2e_url_asserts]]。
