...
- 2026-06-10: **prod 天気取得 Job 有効化（F14 サイネージ天気表示）**（[#799](https://github.com/cometa-kaito/kimiterrace-v2/pull/799) merged）。`cloud_run_job_weather enabled=false→true`。jobs image 626e85c（tv-liveness と共有）/ network(VPC+NAT) / prod-db-url-app secret がすべて揃っており 1行変更のみ。CI 全 pass / fresh Reviewer APPROVE / 自律 merge。**次の作業**: `terraform -chdir=infrastructure/terraform/envs/prod apply` を実行 → Cloud Scheduler（毎時）または手動 `gcloud run jobs execute kimiterrace-weather-fetch --region asia-northeast1 --project signage-v2-prod` で初回取得 → サイネージ天気表示を確認。
