...
- **staging live**: image `web:ac37c9e`（2026-06-10・rev `kimiterrace-web-00001-wvn`）＝最新 main 追従。`/api/health` 200 / `/login` は `private,no-cache`（#783 force-dynamic 反映済）。`migrate_image_tag` 据置（schema 変更なし）。詳細は下記「直近の完了」2026-06-10（デプロイ手順は [web-deploy.md](runbooks/web-deploy.md)）。
