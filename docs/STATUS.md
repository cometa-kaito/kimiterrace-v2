...
- 2026-06-14: **運営整理 v2編「縮退・移設一式」item1〜4 + item3 を出荷（6 PR landed）+ item5(sensors) は次レーン**。正本 = `Desktop/app/運営整理-判定記録-2026-06-13.md` §4 v2編。各項目 ≤500行・別 Reviewer・自律 merge で分割実装:
  ...
  - **残レーン = item5「sensors 全校一本化」**（未着手・次セッション）: ①**登録/編集/撤去を全校(/ops)版に** = 既存 sensor mutation は school_admin スコープ(`SENSOR_WRITE_ROLES`・actor自校id+`tenant_isolation`)。system_admin 版は別物(明示 target school_id + `system_admin_full_access` RLS + `created_by=null`/`actor_identity_uid` 監査パターン = schools/advertisers actions 準拠)。**撤去(decommission)は未実装**(`decommissioned_at` 列はあり、query/action 新設要)。②期間さかのぼり検知履歴(`presence-history-range.ts` 再利用、read-only)。③「応答なし」日次ヘルスチェック+Slack(別PR・infra)。**= cross-tenant RLS(ルール2)に触る security-sensitive な大レーンゆえ PR6(app/db: decommission / register-edit-全校 / history に分割)+PR7(infra: sensor-health Job+Terraform+Slack、tv-liveness 転用)で着手推奨**。
