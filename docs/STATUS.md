...
- 最終更新: 2026-06-14 ／ 更新者: Claude Code
...
- **main HEAD**: `09e09e5`（#897 prod bump）。URL は **namespace 改称完了**で `/ops`(運営)・`/app`(学校) の 2 namespace、`/admin` は物理撤去・旧 `/admin/*` は 308 温存（#891/#894/#895、下記「直近の完了」2026-06-14 / [[ref_namespace_rename_breaks_e2e_url_asserts]]）。
...
- 2026-06-14: **生徒 magic-link 有効期限是正（finding④）完了** [#863](https://github.com/cometa-kaito/kimiterrace-v2/pull/863) merged（squash `04da610`）。`EXPIRES_DEFAULT_DAYS=365` 定数新設・発行 API が `expiresInDays` 省略時も DB 列デフォルト 90 日に倒さずサーバ時刻起点で 365 日を明示算出。`DEFAULT_SIGNAGE_TTL_DAYS` 3650→365（seed）。prod-DB schema/migration は不変更。既存 prod の 10 年リンク（岐南 seed 由来）は既存行のまま（短縮は再 seed 運用判断・別途）。Reviewer APPROVE + CI 13 green + 自律 merge。
