...
- **staging live**: image `web:2b64c28`（2026-06-17・#992 ナビグループ化 / #993 一覧カード化 / #996 ナビアイコン+メトリクスsurface化。64ccbac から現行化＝#986/#989 含む）。`/api/health` 200 / `/login` `private,no-cache`。**migrate 実行済**（`20260615120000`: `academic_year` 列・`ix_classes_school_year` 削除 + `ux_classes_school_grade_name` 新設）。デプロイ手順は [web-deploy.md](runbooks/web-deploy.md)。
- **prod live**: image `web:2b64c28`（2026-06-17・#992 ナビ目的別5グループ化+軸タブ横崩れ修正 / #993 admin一覧モバイルカード化 / #996 ナビにインラインSVGアイコン+ダッシュボードのメトリクスをサーフェス化。1910cd5#989 を supersede・live rev `00058` 突合・`GEMINI_THINKING_BUDGET=0` は env で維持）。`/api/health` 200・`cache-control private,no-cache`。**migrate 実行済**（`20260615120000`・#986/#989 は schema 無変更=追加 migrate なし）。bump 記録 [#988](https://github.com/cometa-kaito/kimiterrace-v2/pull/988)/[#991](https://github.com/cometa-kaito/kimiterrace-v2/pull/991)。
