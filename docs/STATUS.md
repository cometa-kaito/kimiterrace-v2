...
- **staging live**: image `web:7a9d5f8`（2026-06-17・#992 ナビ目的別グループ化 / #993 一覧モバイルカード化。64ccbac から現行化＝#986/#989 含む）。`/api/health` 200 / `/login` `private,no-cache`。**migrate 実行済**（`20260615120000`: `academic_year` 列・`ix_classes_school_year` 削除 + `ux_classes_school_grade_name` 新設）。デプロイ手順は [web-deploy.md](runbooks/web-deploy.md)。
- **prod live**: image `web:7a9d5f8`（2026-06-17・#992 sysadminナビ目的別5グループ化+軸タブのモバイル横崩れ修正 / #993 admin一覧モバイルカード化。1910cd5#989 を supersede・live rev `00057` 突合・`GEMINI_THINKING_BUDGET=0` は env で維持）。`/api/health` 200・`cache-control private,no-cache`。**migrate 実行済**（`20260615120000`・#986/#989 は schema 無変更=追加 migrate なし）。bump 記録 [#988](https://github.com/cometa-kaito/kimiterrace-v2/pull/988)/[#991](https://github.com/cometa-kaito/kimiterrace-v2/pull/991)。
...
- 2026-06-17: **デザイン刷新「sysadminナビのグループ化 + 一覧のモバイルカード化」を staging→prod 反映済み（夜間自律: 実装→fresh Reviewer→CI→自律 merge→prod、prod live `web:7a9d5f8` rev `00057`）**。2026-06-16 のデザインレビュー（PC/モバイル プレビュー検討）でのユーザー指摘 ①sysadmin の 16 項目フラットナビが走査しづらい ②ダッシュボード軸タブのモバイル横崩れ への対応。
  ...
  - bump 記録 [#994](https://github.com/cometa-kaito/kimiterrace-v2/pull/994)（staging 64ccbac→7a9d5f8 / prod 1910cd5→7a9d5f8）。UI/CSS のみ・**schema/secret 無変更=migrate 不要**。検証: prod/staging `/api/health` 200・`/login` `private,no-cache`（force-dynamic 健全）・apply 0add/1change/0destroy。各 PR fresh Reviewer APPROVE + CI 全緑 + 自律 squash merge。
  - **保留（朝に共同レビュー推奨）**: ①ナビ各項目の**アイコン**（プロトタイプにはあるが実装にアイコン基盤が無く、依存追加 or インラインSVG の方針判断が必要なため未着手）。②メトリクスカード/見出し/余白の微ポリッシュ（PR3 候補・低リスク）。③**サイネージ盤面/エディタの見た目変更は非対象**（学校が毎朝見る高リスク面ゆえ意図的に触らない）。④一覧カード化の**スクリーンリーダ実機確認**（display:block 化の既知トレードオフ・ユーザー本人ログイン要）。
