...
- **staging live**: image `web:57dce88`（2026-06-17 #998/#999 system_admin が /ops/schools/[id]/hierarchy で特定校のクラス設定を編集可に＝今回反映・schema/secret 無変更=migrate不要。前回 #992 ナビグループ化 / #993 一覧カード化 / #996 ナビアイコン+メトリクスsurface化。64ccbac から現行化＝#986/#989 含む）。`/api/health` 200 / `/login` `private,no-cache`。**migrate 実行済**（`20260615120000`: `academic_year` 列・`ix_classes_school_year` 削除 + `ux_classes_school_grade_name` 新設）。デプロイ手順は [web-deploy.md](runbooks/web-deploy.md)。
- **prod live**: image `web:57dce88`（2026-06-17 #998/#999 system_admin が /ops/schools/[id]/hierarchy で特定校のクラス設定を編集可に＝今回反映・**schema/secret 無変更=migrate不要**・live image=web:57dce88 突合・新ルート unauth 307→/login。2b64c28(#996) を supersede・`GEMINI_THINKING_BUDGET=0` は env で維持）。`/api/health` 200・`cache-control private,no-cache`。**migrate 実行済**（`20260615120000`・#998/#999/#986/#989 は schema 無変更=追加 migrate なし）。bump 記録 [#1000](https://github.com/cometa-kaito/kimiterrace-v2/pull/1000)。
