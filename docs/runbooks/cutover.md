...
### Phase C — F15 TV デバイス + SWITCHBOT シークレット rotation（人間 = Desktop 実施）

手順は [switchbot-secret-rotation メモリ](../../.claude/projects/.../memory/project_switchbot_secret_rotation.md) の 3 ステップに準拠（公開リポジトリに `SWITCHBOT_WEBHOOK_SECRET` が平文露出、cutover に rotation 一式を集約と決定済）。
