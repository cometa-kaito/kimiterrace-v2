...
> **▶ prod をゼロから建てて岐南工業の実機 TV を端末操作ゼロで v2 へ切り替える具体手順は
> [prod-bringup-cutover.md](prod-bringup-cutover.md)（本番(prod)構築 + 岐南 実機TV cutover の機械的チェックリスト）。**
> 本ページは本体アプリ + データ（Firestore → Cloud SQL）の本番移行・段階 DNS・旧 Firebase 停止の広い文脈を扱う。
> prod 環境 bring-up（Terraform enabled 化 / 2 段 apply / migrate + 岐南 seed）と LP→v2 の TV 切替（LP-compat / device_id 取得 / TV_POLL_SECRET 一致 / LP-proxy or DNS）は上記の専用 runbook を参照。
...
- **[prod-bringup-cutover.md](prod-bringup-cutover.md): 本番(prod)構築 + 岐南 実機TV cutover の機械的チェックリスト（prereq / bring-up / device_id 取得 / TV_POLL_SECRET 一致 / LP-proxy or DNS / 検証 / ロールバック）**
