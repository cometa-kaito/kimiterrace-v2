...
| 残対策（follow-up） | authoring 時の入力ガード（掲示物 publish 経路に決定論ヒューリスティック soft-gate = warn + override + 監査）＋コンテンツポリシー強化を方針確定（[ADR-030](../adr/030-authoring-time-pii-gate.md)）。ML 日本語人名 NER は FP/コスト計測後の follow-up。実装スライスは **issue #426** で追跡。|
