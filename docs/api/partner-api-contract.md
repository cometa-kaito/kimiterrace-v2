...
## 3.5 K4 — 学校階層 pull（read-only・運営整理 Phase6）

```
GET /api/partner/schools/{schoolId}/hierarchy
```
- `schoolId` = v2 `schools.id`（UUID）。portal は名寄せ済の `schools.v2_school_id` を渡す（名前一致禁止・橋キーは UUID）。
- 用途: portal「学校営業台帳」が**運用中校を v2 正本で自動表示・編集ロック**するための「学校 → 設置場所 → モニタ」階層 read。`system_admin` context（cross-tenant）で実行。
- 階層モデル（統合マスター §0b）: v2 に独立した「設置場所」テーブルは無い。物理最小単位は**モニタ = `tv_devices` 1 行**で、その `label`（自由文字列・PII 不可）が設置場所を表す。設置場所単位のグルーピング（中間層）は portal の表示側に委ねる。
- **200**:
```jsonc
{
  "school_id": "uuid",
  ...
  "prefecture": "…",
  "code": "…|null",
  "hierarchy_mode": "class|department",
  "monitors": [
    {
      "id": "uuid",                 // tv_devices.id（露出可。ポーリング解決キー device_id とは別物）
      "label": "電子工学科 1年|null", // 設置場所ラベル（PII 不可）
      "grade_name": "…|null",
      "department_name": "…|null",  // 学科モード校でのみ非 null
      "class_name": "…|null",
      "status": "online|quiet|down|never", // v2 が正本判定（last_seen 鮮度・F16）
      "last_seen_at": "…Z|null",
...
```
- ⚠️ **秘匿値・PII を返さない**（契約 §0）: `device_id`（推測不能なポーリング解決キー）・`target_mac`・`fcm_token`・`signage_url`・`webhook_url` は**返さない**。返すのは設置場所ラベルと稼働メタ（公開可の集計的状態）のみ。
- 並びは設置場所ラベル → モニタ id で決定的。ソフトデリート済（`deleted_at`）のモニタは出さない。
- エラー: 401（未設定/不一致）/ 400（id 形式不正）/ 404（学校無 = RLS 不可視/不存在）/ 500。冪等・GET。

## 3.6 K5 — 学校一覧 pull（read-only・運営整理 Phase6 名寄せ）

```
GET /api/partner/schools
```
- 用途: portal「学校営業台帳」の **名寄せ（reconcile）**。portal は未名寄せの自校（商流 schools）と v2 の学校一覧を突き合わせ、候補を自動提示する。**確定（`schools.v2_school_id` への UUID 書き込み）は人間の一括確認**で行う（校名での自動リンクは禁止＝別校データ誤表示の事故防止）。本エンドポイントは候補の母集合を供給するだけ。
- `system_admin` context（cross-tenant）で全校マスタを read。引数なし。
- **200**:
```jsonc
{
  "schools": [
    {
      "id": "uuid",                 // v2 schools.id（portal はこれを v2_school_id に保存）
      "name": "…",
      "prefecture": "…",
      "code": "…|null",            // 文科省標準学校コード（任意）
      "hierarchy_mode": "class|department"
    }
  ],
  "generated_at": "…Z",
...
```
- ⚠️ **秘匿値・PII を返さない**（契約 §0）: 学校マスタの公開的属性（校名・都道府県・コード・階層モード）のみ。監査列（created_at 等）も射影外。
- 並びは都道府県 → 校名 → id で決定的（`listSchools` を再利用）。
- エラー: 401（未設定/不一致）/ 500。一覧なので 404 は無い（0 件は空配列 `[]`）。冪等・GET。
...
3. **K4**（低リスク・read のみ・schema 変更なし）: `packages/db` の `getSchoolHierarchy` クエリ → `app/api/partner/schools/[id]/hierarchy/route.ts`（K1 metrics と同型の認証・system_admin context）→ テスト。
4. **K5**（最小・read のみ・新クエリ不要）: 既存 `listSchools` を再利用し `app/api/partner/schools/route.ts`（K4 と同型の認証・system_admin context）→ テスト。portal 名寄せ UI の母集合供給。
