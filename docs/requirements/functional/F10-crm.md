...
- [~] advertisers テーブル: 会社名、担当者、連絡先、ステータス（見込/契約中/休止）、業種 — 部分実装（[#270](https://github.com/cometa-kaito/kimiterrace-v2/pull/270)、`packages/db/src/schema/advertisers.ts`、UI: list/new/edit/toggle）残: 現状ステータスは `is_active` boolean（論理削除）のみ。**2026-06-03 ユーザー確定: 仕様通り 3 状態 enum（見込 prospect / 契約中 active / 休止 paused）に拡張する**（pgEnum 化 + migration + 監査、`is_active` は enum へ移行 or 併存判断は実装スライスで。F09 広告主レポートの契約状況表示と整合。型は Drizzle 単一ソース＝[CLAUDE.md ルール3](../../../CLAUDE.md)、enum 末尾追加時の consumer narrow ドリフトに注意）
