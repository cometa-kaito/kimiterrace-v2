...
- 状態: MVP 実装済（入力 UI（音声/テキスト）+ 履歴一覧 + 抽出トリガ + **抽出結果 → 下書き → 編集/公開の橋渡し** [#524](https://github.com/cometa-kaito/kimiterrace-v2/pull/524)）。残: Cloud Speech-to-Text フォールバック（別 issue）/ 実 Vertex 構造化呼出の有効化（PII マスキング基盤 [#289](https://github.com/cometa-kaito/kimiterrace-v2/issues/289) は実装済、実呼び出しは後続）
...
- [x] 抽出結果は [F01](F01-teacher-file-extraction.md) と同じ編集 UI に流れる — 実装済（[#524](https://github.com/cometa-kaito/kimiterrace-v2/pull/524)）: 音声/チャット入力も履歴の「編集して公開」から `createDraftFromInputAction` で下書き content を生成し、F01 と同一の既存エディタ `/admin/contents/[id]` へ遷移（transcript を本文に materialize、構造化フィールド自動充填は実 Vertex 呼び出し有効化後の enhancement）
