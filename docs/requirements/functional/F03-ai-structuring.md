...
- 関連 ADR: ADR-005 (Vertex AI), ADR-017 (起票予定: confidence_score 必須化含む)
...
- [ ] プロンプト・応答・トークン数・確信度を audit_log の `ai_extractions` テーブルに記録
- [ ] レート制限: school_id あたり 1 分 60 リクエスト
- [ ] プロンプトインジェクション対策: ユーザー入力は system プロンプトを上書きできない構造（XML タグでセパレート）
