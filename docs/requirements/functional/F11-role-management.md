...
- 状態: 実装中（無効化/再有効化・ロール変更・全校横断/自校 教職員一覧・認可ポリシー・last-admin 多層防御・MFA enrollment capability は実装済。新規アカウント発行=初期ロール任命の操作系と MFA 本番強制 ON は未実装）
- 関連 ADR: ADR-003 (Identity Platform), [ADR-026 (無効化/ロール変更エンフォース経路)](../../adr/026-account-deactivation-role-change-enforcement.md), [ADR-031 (MFA 段階的エンフォース)](../../adr/031-mfa-phased-enforcement.md)
...
- [~] MFA（teacher 以上、[NFR03](../non-functional/NFR03-security.md)）— **2026-06-03 ユーザー確定: 段階的エンフォース（[ADR-031](../../adr/031-mfa-phased-enforcement.md)）**。capability（IdP `mfa_config` 有効化 + アプリ側の multiFactor enrollment フロー）を MVP で実装し、岐南工業 PoC では**任意**（教員の初期導入摩擦を避ける）、**本番導入ゲートで teacher 以上に強制化**して NFR03 を満たす。
  - **アプリ enrollment capability 実装済（#47, ADR-031）**: teacher 以上が自分の **TOTP 第2要素**を登録/解除する画面 `/admin/account/mfa`（`apps/web/app/admin/account/mfa/`、`requireRole(MFA_REQUIRED_ROLES)`）。登録/解除の成否を `audit_log` に記録（actor=自分・件数のみ・電話番号等 PII 非記録、ルール1/ルール4、`apps/web/lib/mfa/enrollment-actions.ts`）。factor=TOTP 採用（SMS の電話番号 PII/コストを回避、ADR-031 §決定「TOTP 既定」）。enrollment/監査の真実は IdP（Admin SDK 再読、`apps/web/lib/auth/mfa-admin.ts`、クライアント自己申告を信用しない）。
  - **強制ゲート（既定 OFF）実装済（#47, ADR-031）**: `MFA_ENFORCEMENT=on` のときだけ、未登録 teacher 以上を `/admin` レイアウトで enrollment へ誘導（`apps/web/lib/mfa/policy.ts` / `enforce-gate.ts`、`apps/web/app/admin/layout.tsx`）。**既定 OFF＝IdP も叩かず誘導もしない＝既存ログイン挙動が不変（PoC 非強制、ADR-031 §2、回帰なし）**。enrollment ページ配下ではループ防止で誘導しない。現在パスは middleware が注入する `x-kt-pathname` ヘッダから読む（純加算的変更）。IdP 一時障害時は可用性優先で通す fail-safe。本番導入ゲートで env を `on` にして強制化する（エンフォースの単一ソースは IdP、ADR-026/ADR-031 §4）。
  - **残**: Terraform `identity_platform` の `mfa_config` 有効化（[#541](https://github.com/cometa-kaito/kimiterrace-v2/pull/541)、別レーン）/ 本番導入ゲートで `MFA_ENFORCEMENT=on` 切替 + grace period 確定（導入計画）/ 実 IdP E2E（Phase 検証③）。
