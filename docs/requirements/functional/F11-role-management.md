...
- [ ] MFA 強制（teacher 以上、[NFR03](../non-functional/NFR03-security.md)）— 未実装（Terraform identity_platform は `mfa_config` 未設定の TODO、アプリ側にも multiFactor enrollment/enforcement なし）
- [ ] アカウント無効化時に既存 magic_link は失効しない（クラス単位の link は教員紐付けではないため）— 暗黙成立（`deactivateIdpUser` は IdP disable + revokeRefreshTokens のみで `magic_links` を touch せず、magic link は student class-level token として独立。ただしこれを pin する回帰テストは未確認のため box は据え置き）
