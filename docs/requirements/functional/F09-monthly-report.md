...
- [~] system_admin UI からダウンロード可能 — 部分実装（[#313](https://github.com/cometa-kaito/kimiterrace-v2/pull/313)、[#359](https://github.com/cometa-kaito/kimiterrace-v2/pull/359)）。**自校 publisher 向けの月次 CSV DL ルート（旧 `apps/web/app/api/reports/monthly/route.ts`）は #902（§43）で self-school ダッシュボード/レポート retire に伴い撤去済**。現存するのは `/ops/reports`（`apps/web/app/ops/reports/page.tsx`）の保存済 PDF 履歴ビューのみ。残: system_admin 向け cross-tenant DL・PDF DL は未実装
